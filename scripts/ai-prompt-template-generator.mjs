...
export const PROMPT_TEMPLATE_SPECS = {
  translation: {
    ...
    sections: {
      ...
      concordanceHeader: 'concordance-header',
      concordanceEntrySummary: 'concordance-entry-summary',
      concordanceEntrySource: 'concordance-entry-source',
      concordanceEntryTarget: 'concordance-entry-target',
      tbHeader: 'tb-header',
      tbEntry: 'tb-entry',
...
function ensureNoExtraneousContent(sectionBody, fileLabel, sectionId) {
  const bodyWithoutTextFences = sectionBody.replace(TEXT_FENCE_PATTERN, '').trim();
...
export function parseMarkdownSections(markdown, fileLabel) {
  ...
  for (let index = 0; index < matches.length; index += 1) {
    ...
    const sectionEnd =
      index + 1 < matches.length ? matches[index + 1].index : normalizedMarkdown.length;
    const sectionBody = normalizedMarkdown.slice(sectionStart, sectionEnd).trim();
