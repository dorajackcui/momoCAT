...
test('forbidden specifier patterns match raw desktop imports', async () => {
  const { matchesForbiddenSpecifierPatterns } = await import('./gate-architecture-check.mjs');
  ...
  assert.equal(matchesForbiddenSpecifierPatterns('apps/desktop/src/shared/ipc', patterns), true);
