...
const requests: Array<{ channel: string; args: unknown[] }> = [
  ...
  { channel: segment.update, args: ['seg-1', [], 'new', 'request-1'] },
  { channel: file.export, args: [1, 'output.xlsx', importOptions, false] },
...
describe('IPC argument boundaries', () => {
  ...
  it.each([
    ...
    [tb.mount, [1, 'tb-1', NaN]],
    [file.export, [1, 'output.xlsx', undefined, 'false']],
...
describe('valid IPC payload compatibility', () => {
  it.each([
    ...
    [file.export, 'exportFile', [1, 'output.xlsx'], [1, 'output.xlsx', undefined, false]],
    [file.export, 'exportFile', [1, 'output.xlsx', importOptions, true]],
