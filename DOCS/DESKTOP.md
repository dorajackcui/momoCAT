...
The segment-update handler validates segment identity, token shapes, status, and optional request identity before calling the service. Its rejection and valid-payload contract is exercised by [projectHandlers.test.ts](../apps/desktop/src/main/ipc/projectHandlers.test.ts). Other handlers still need their own runtime validation; registration and preload type tests alone do not prove that an untrusted payload is safe.
