...
The Project Translation Memory tab keeps Working TM management intentionally narrow: users can export its source/target rows to XLSX or reset all entries after confirmation. Reset runs in a background worker, preserves the TM resource, project mount, project files, and translated segments, then invalidates TM reference caches.
