...
- `segments.qaIssuesJson` stores the last persisted QA findings, including optional group identity/label, terminology origins, and reference rows. NULL means no current result; an empty JSON array means the check found no issues. Findings are advisory; old persisted blocking metadata has no consumer and no effect on operations. Consumers display all findings as QA problems; legacy severity fields remain readable. Both full and instant checks write here. Result retention and configuration/terminology invalidation are owned by [QA result lifecycle](LOCALIZATION.md#qa-result-lifecycle-and-legacy-consumers).
