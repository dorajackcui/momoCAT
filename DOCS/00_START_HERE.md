...
- Default resume identity includes the input filename, output basename, project id, and translation policy fingerprint. Unit checkpoint hashes also include that fingerprint, so `--resume` ignores stale records when project, target-scope, mode, provider/model, reasoning, temperature, or system prompt override changes.
